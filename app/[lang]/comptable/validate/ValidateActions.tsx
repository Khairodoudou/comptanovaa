...
export function ValidateActions({ entryId, validatorId, t }: { entryId: string; validatorId: string; t: ValidateT }) {
  ...
  async function handleAction(action: "VALIDATED" | "REJECTED") {
    ...
    try {
      const res = await fetch(`/api/comptable/entries/${entryId}`, {
        method: "PATCH",
  ...
  return (
    <>
      <div className="flex items-center gap-2">
        <button id={`validate-${entryId}`} onClick={() => handleAction("VALIDATED")} disabled={!!loading}
        ...
        <button id={`reject-${entryId}`} onClick={() => setShowRejectModal(true)} disabled={!!loading}
      ...
      {showRejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4">
            ...
            <textarea id={`reject-comment-${entryId}`} value={comment} onChange={(e) => setComment(e.target.value)}
